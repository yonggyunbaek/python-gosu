...
XYZ 마트에서 14일간 회원을 대상으로 할인하는 제품이 날짜 순서대로 치킨, 사과, 사과, 바나나, 쌀, 사과, 돼지고기, 바나나, 돼지고기, 쌀, 냄비, 바나나, 사과, 바나나인 경우에 대해 알아봅시다. 첫째 날부터 열흘 간에는 냄비가 할인하지 않기 때문에 첫째 날에는 회원가입을 하지 않습니다. 둘째 날부터 열흘 간에는 바나나를 원하는 만큼 할인구매할 수 없기 때문에 둘째 날에도 회원가입을 하지 않습니다. 
...
def solution(want, number, discount):
    want_dict = dict(sorted(zip(want, number)))
    ans = 0
    for i in range(len(discount) - 9):
        product_count = defaultdict(int)
        
        for j in range(i, i + 10):
            product_count[discount[j]] += 1
        
        if all(product_count.get(p, 0) >= n for p, n in want_dict.items()):
            ans += 1
